/**
 * MOCK BACKEND — everything in the browser, nothing on a server.
 *
 * This is what runs when no Supabase keys are configured, so the app still
 * works as a demo. `dataClient.js` picks between this and `liveClient.js`.
 * -----------------------------------------------------------------------------
...
export const mockApi = {
  ...
  async createBrandAsset(
    session,
    { section_id, kind, title, body, url, file_name, file_size, size, parent_id = null, columns, file }
  ) {
    assertAdmin(session)
    // No server to upload to: the file becomes a data URL in this snapshot.
    if (file) ({ url, file_name, file_size } = await readFileAsDataUrl(file))
  ...
  async updateBrandAsset(session, id, patch) {
    ...
    if (patch.file) {
      const read = await readFileAsDataUrl(patch.file)
      patch = { ...patch, ...read }
    }
  ...
  /** No auth to speak of in the mock — the dev toggle stands in for it. */
  async signIn() {},
  async signOut() {},
  onAuthChange() {
    return () => {}
